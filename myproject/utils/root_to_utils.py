...
def create_bg_np(
    ...
    if eng_mask > 0:
        img_np = np.concatenate([img_np, np.zeros((h, w, 5))], axis=-1)
    if with_time:
...
def load_rgb(
    ...
    if eng_mask > 0.0:
        # 生成一个长度为n_hit的True/False列表，其中True的概率为eng_mask
        if eng_mask > 1.0: eng_mask = 1.0
        true_count = int(n_hit * eng_mask)
        false_count = n_hit - true_count
        mask_list = [True] * true_count + [False] * false_count
        random.shuffle(mask_list)
    else:
        mask_list = [False] * n_hit
    ...
    for i in range(n_hit):
        ...
        if eng_mask > 0.0:
            img_np[ymin:ymax, xmin:xmax, 3:6] = np.array([b, g, r])
            img_np[ymin:ymax, xmin:xmax, 6] = m_eng[i]
            img_np[ymin:ymax, xmin:xmax, 7] = 1.0 if mask_list[i] else 0.0
        if with_time:
            img_np[ymin:ymax, xmin:xmax, -1] = m_time[i] + 1    # {0, 1, 2, ..., 20} -> {1, 2, 3, ..., 21}
