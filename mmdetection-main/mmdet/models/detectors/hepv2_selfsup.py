...
class HEPv2SelfSupervisor(BaseModel, metaclass=ABCMeta):
    ...
    def loss(self, inputs: torch.Tensor,
             data_samples: OptSampleList = None) -> dict:
        ...
        pred, target, mask = self.head(x)
        losses = self.head.loss(pred, target, mask)
        return losses
