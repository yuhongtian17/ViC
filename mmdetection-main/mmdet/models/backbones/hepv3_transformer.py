...
class HEPv3Transformer(BaseModule):
    ...
    def __init__(
        ...
        self.output_feature_maps = (self.mix_mode in ['pre', 'post'])
        if self.output_feature_maps: assert depth == sum(vic_cfg['depths'])

        self.vic = MMDET_VHEAT(
            patch_size=4,
            in_chans=3,
            num_classes=1000,
            depths=vic_cfg['depths'],
            dims=vic_cfg['dims'],
            drop_rate=0.,
            attn_drop_rate=0.,
            drop_path_rate=vic_cfg['drop_path_rate'],
            norm_layer=nn.LayerNorm,
            patch_norm=True,
            post_norm=vic_cfg['post_norm'],
            layer_scale=None,
            use_checkpoint=vic_cfg['use_checkpoint'],
            out_indices=vic_cfg['out_indices'],
            pretrained=None,
            img_size=vic_cfg['img_size'],
            feat_fusion_mode=vic_cfg['feat_fusion_mode'],
            output_feature_maps=self.output_feature_maps,
            output_feature_map_indices=output_feature_map_indices,
    ...
    def forward(self, batch_inputs):
        ...
        x_the = x[..., 3]
        # x_time = x[..., 4]
        x_grid = x[..., 5:7].unsqueeze(-2)  # B, N, 1, 2
        flags_0 = x[..., 0]
        ...
        if self.output_feature_maps:
            vic_outs, vic_all_feature_maps = self.vic(inputs)
        else:
            vic_outs = self.vic(inputs)

        outs = []
        for i, blk in enumerate(self.blocks):
            if self.output_feature_maps:
                vic_feature_map = vic_all_feature_maps[i]
                if vic_feature_map is not None:
                    sampled = F.grid_sample(
                        vic_feature_map,
                        x_grid,
                        mode='bilinear',        # 双线性采样
                        padding_mode='zeros',
                        align_corners=True,     # 确保坐标原点与像素角点对齐
                    )                           # 输出形状: (B, C, N, 1)
                    sampled = sampled.squeeze(-1).permute(0, 2, 1)
                    if self.mix_mode == 'pre':
                        x = blk(self.mix_N(x, sampled, N), attn_mask)
                    elif self.mix_mode == 'post':
                        x = self.mix_N(blk(x, attn_mask), sampled, N)
                    else:
                        x = blk(x, attn_mask)
                else:
                    x = blk(x, attn_mask)
        ...
        if self.out_vic:
            return outs, outs_others, vic_outs
        else:
            return outs, outs_others

    def mix_N(self, x, sampled, N):
        return torch.cat([
            x[:, :N, :] + sampled[:, :N, :],
            x[:, N:, :],
        ], dim=1)
