...
class HEPv2SelfSupervisorCLSHead(BaseModule):
    ...
    def loss_single(self,
                    ...
                    mask: torch.Tensor):
        if loss_name == 'eng':
            num_classes = self.num_classes_eng
            loss_model = self.loss_eng
        elif loss_name == 'phi':
            num_classes = self.num_classes_phi
            loss_model = self.loss_phi
        elif loss_name == 'the':
            num_classes = self.num_classes_the
            loss_model = self.loss_the
        else:
            raise NotImplementedError

        _, pred_top1 = torch.max(pred, dim=-1)
        pred_top1 = pred_top1.to(dtype=torch.long).detach()
        correct_mask = (pred_top1[mask] == target[mask])
    ...
    def loss(self, pred, target, mask):
        if self.recover_eng:
            mask_eng = mask[0]
            loss_eng, acc_eng = self.loss_single('eng', pred[0], target[0], mask_eng)
        else:
        ...
        if self.recover_phithe:
            mask_phi = mask[1]
            mask_the = mask[2]
            loss_phi, acc_phi = self.loss_single('phi', pred[1], target[1], mask_phi)
            loss_the, acc_the = self.loss_single('the', pred[2], target[2], mask_the)
        else:
