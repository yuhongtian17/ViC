...
class HEPv2SelfSupervisorREGHead(BaseModule):
    ...
    def __init__(
        ...
        if self.recover_eng:
            self.classifier_eng = nn.Linear(self.embed_dim, 1, bias=True)
            self.loss_eng = MODELS.build(loss_eng)

        if self.recover_phithe:
            self.classifier_phi = nn.Linear(self.embed_dim, 1, bias=True)
            self.classifier_the = nn.Linear(self.embed_dim, 1, bias=True)
